...
struct EventStreamDemoView: View {
    ...
    private func startEventMonitoring() async {
        ...
        Task { @MainActor in
            // Note: In real implementation, events stream would be available
            // For now, we simulate with state changes
            addEvent("Event monitoring started", details: "Listening for player events", type: .info)
            
            // Monitor state changes as example
            for await state in await service.stateUpdates {
                ...
                addEvent(eventName, details: "State: \(state)", type: eventType)
            }
        }
    }
