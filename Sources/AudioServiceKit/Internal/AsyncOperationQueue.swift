...
actor AsyncOperationQueue {
    ...
    #if DEBUG && ENABLE_QUEUE_DIAGNOSTICS
    /// Get queue diagnostics (DEBUG-only, requires ENABLE_QUEUE_DIAGNOSTICS flag)
...
// MARK: - Queue Diagnostics (DEBUG-only with ENABLE_QUEUE_DIAGNOSTICS flag)

/// Comprehensive queue diagnostics for debugging and performance analysis
/// Only compiled when both DEBUG and ENABLE_QUEUE_DIAGNOSTICS flags are set
