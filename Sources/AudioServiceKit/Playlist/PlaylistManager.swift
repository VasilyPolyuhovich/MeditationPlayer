...
actor PlaylistManager {
    ...
    func load(tracks urls: [URL]) {
        ...
        if validTracks.count < urls.count {
            Self.logger.debug(" Warning: \(urls.count - validTracks.count) tracks filtered out (files not found)")
        }
    ...
    func addTrack(_ url: URL) {
        guard let track = Track(url: url) else {
            Self.logger.debug(" Warning: Track not added - file not found: \(url.lastPathComponent)")
            return
        }
        tracks.append(track)
    }
    ...
    func insertTrack(_ url: URL, at index: Int) {
        guard let track = Track(url: url) else {
            Self.logger.debug(" Warning: Track not inserted - file not found: \(url.lastPathComponent)")
            return
        }
    ...
    func validateTrack(_ track: Track) async -> Bool {
        ...
        guard FileManager.default.fileExists(atPath: track.url.path) else {
            Self.logger.debug(" Track validation failed: file not found - \(track.url.lastPathComponent)")
            return false
        }
        ...
        do {
            ...
            Self.logger.debug(" Track validation failed: \(track.url.lastPathComponent), error: \(error.localizedDescription)")
            return false
        }
    }
    ...
    func skipToTrackWithRetry(
        ...
        // Save original index for rollback
        ...
        while attempts < maxAttempts {
            ...
            // Peek at candidate track
            guard let candidateTrack = peekInDirection(direction) else {
                Self.logger.debug(" No more tracks in \(direction) direction")
            ...
            if isValid {
                ...
                Self.logger.debug(" Skip successful (attempt \(attempts)): \(candidateTrack.url.lastPathComponent)")
                return candidateTrack
            } else {
                // Validation failed - advance to skip corrupted file
                Self.logger.debug(" Attempt \(attempts) failed, trying next track...")
                _ = skipInDirection(direction)

                // Detect infinite loop (wrapped back to start)
                if currentIndex == originalIndex {
                    Self.logger.debug(" Wrapped around - all tracks invalid")
                    restoreIndex(originalIndex)
                    return nil
                }
            }
        ...
        Self.logger.debug(" All \(maxAttempts) attempts failed")
        restoreIndex(originalIndex)
        return nil
    }
